...
- **Missions**: Launch multi-agent projects where an orchestrator coordinates specialized worker droids to plan, implement, and validate larger features. Start one with `/missions` or run headlessly with `droid exec --mission`. [Learn more →](/cli/features/missions)
