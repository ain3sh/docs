...
私たちの**Trust Center**では、最新のレポート、セキュリティアーキテクチャドキュメント、およびサブプロセッサーリストを提供しています。セキュリティとコンプライアンスのレビューの主要なリファレンスとしてご利用ください。
...
独自のOTLPコレクターへのデュアルエクスポートの段階的な設定については、[Telemetry Export (OTEL)](/enterprise/telemetry-export)をご参照ください。

コストと生産性分析に有用なメトリクスとトレースの詳細については、[Usage, Cost & Productivity Analytics](/enterprise/usage-cost-and-analytics)をご参照ください。
...
この設定の大部分は、[Hierarchical Settings & Org Control](/enterprise/hierarchical-settings-and-org-control)で説明されている階層設定システムを通じて表現されます。