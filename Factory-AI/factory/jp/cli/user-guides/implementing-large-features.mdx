---
title: "大規模機能の実装"
description: 仕様計画と反復実装を使って、複雑で複数フェーズの開発プロジェクトに取り組む体系的な方法。
...
<CardGroup cols={2}>
  <Card title="体系的な計画" icon="map">
    複雑な機能を、明確な境界を持つ管理しやすい個別フェーズに分割します
  </Card>
  <Card title="段階的な進捗" icon="stairs">
    各ステップで検証とテストを行いながら、一度に1フェーズずつ実装します
  </Card>
  <Card title="バージョン管理戦略" icon="code-branch">
    gitコミットとPRで進捗を追跡し、安全なロールバックを可能にします
  </Card>
  <Card title="継続的な検証" icon="check-circle">
...
<Steps>
  <Step title="マスタープランを作成">
    仕様モードを使用して、プロジェクトを主要フェーズに分割する包括的なドキュメントを作成します
    プロジェクトを主要フェーズに分割します。
  </Step>
  <Step title="フェーズごとの実装">
    マスタープラン文書を参照しながら、各フェーズごとに新しいセッションを開始します。
  </Step>
  <Step title="頻繁なコミットとPR">
    完了した各フェーズに対応するgitコミットとプルリクエストを作成します。
  </Step>
  <Step title="段階的な検証">
    最後ではなく各フェーズ後に機能をテスト・検証します。
  </Step>
  <Step title="計画を更新">
    完了したフェーズをマークし、学びに基づいて残りの作業を調整します。
  </Step>
</Steps>
