...
再度`/hooks`を実行するか、`~/.factory/hooks.json`を開いて設定を確認します：
...
* 包括的なセキュリティのベストプラクティスと安全ガイドラインについては、フックリファレンスドキュメントの[セキュリティ上の考慮事項](/jp/reference/hooks-reference#security-considerations)を参照してください。
* トラブルシューティング手順とデバッグテクニックについては、フックリファレンスドキュメントの[デバッグ](/jp/reference/hooks-reference#debugging)を参照してください。
