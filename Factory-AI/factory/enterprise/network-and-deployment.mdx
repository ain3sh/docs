...
- **System-managed `settings.json`** – for MDM-deployed and airgapped fleets, drop a settings file at a hardcoded platform path so org policy is in effect before any user authenticates. See [Hierarchical Settings & Org Control](/enterprise/hierarchical-settings-and-org-control#system-managed-settings-file).
