...
1. **Directly configured gateway endpoints**
   ...
   - Custom models configured via [BYOK](/cli/byok/overview) can point to gateway endpoints using `base_url` and provider‑specific settings, so the same gateway policy applies whether models are built‑in or custom.
...
For concrete examples of configuring custom models (including gateway‑backed models), see [Custom models & BYOK](/cli/byok/overview), which covers the `customModels` array in `~/.factory/settings.json` and how those models appear in the `/model` selector.
...
- **Remote and locked-down environments**
  ...
  - Supporting Factory App and browser-based interfaces for secure enterprise deployments.
