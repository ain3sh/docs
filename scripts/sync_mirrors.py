...
MIRRORS_FILE = REPO_ROOT / "mirrors.json"
...
- Update `mirrors.json` (it follows [`mirrors.schema.json`](./mirrors.schema.json)) directly in the GitHub web editor or via PR to add/remove mirrors.
...
- To sync immediately, run `python3 scripts/sync_mirrors.py` locally (use `--only owner/repo` to target a single mirror).
